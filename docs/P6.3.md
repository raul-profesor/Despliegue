...
7. Corremos el contenedor indicando las credenciales de nuestra cuenta LDAP mediante variables de entorno con la flag `-e`. Para este caso, vamos a probar un servidor LDAP externo, csimulando que tuviéramos que integrar nuestro despliegue con un servidor ya existente en la empresa. 
   
     Utilizaremos un servidor público en Internet dedicado a pruebas: [https://www.freeipa.org/page/Demo](https://www.freeipa.org/page/Demo). En esa misma URL podéis consultar usuarios y contraseñas.
...
8.  No nos queda más que visitar `http://IP-EC2:3000/demo`. Si todo ha ido bien, nos solicitará nuestras credenciales para loguearnos contra el servidor openldap.
